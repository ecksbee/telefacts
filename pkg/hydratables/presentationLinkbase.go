...
func HydrateEmbeddedPresentationLinkbase(file *serializables.SchemaFile, fileName string) (*PresentationLinkbase, error) {
	if len(fileName) <= 0 {
		return nil, fmt.Errorf("empty file name")
	}
	if file == nil {
...
func hydrateEmbeddedPresentationLink(schemaFile *serializables.SchemaFile) []PresentationLink {
	...
	for _, annotation := range schemaFile.Annotation {
		for _, appInfo := range annotation.Appinfo {
			for _, linkbase := range appInfo.EmbeddedLinkbase {
				for _, link := range linkbase.PresentationLink {
